...
/**
 * Clase ControladorPanelAnaydirSalasEnExposicionesPermanentesDadasDeAltaAdministrador
 * 
 */
public class ControladorPanelAnaydirSalasEnExposicionesPermanentesDadasDeAltaAdministrador implements ActionListener {
	...
	/**
	 * Constructor ControladorPanelAnaydirSalasEnExposicionesPermanentesDadasDeAltaAdministrador
	 * @param vista (Ventana)
	 */
	public ControladorPanelAnaydirSalasEnExposicionesPermanentesDadasDeAltaAdministrador(Ventana vista) {
		this.vista = vista;
		table = vista.getPanelAnaydirSalasEnExposicionesPermanentesDadasDeAltaAdministrador().getTable();
		model = vista.getPanelAnaydirSalasEnExposicionesPermanentesDadasDeAltaAdministrador().getModel();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand() == "Agregar") {
			if (table.getSelectedRow() == -1) {
				...
				agregarSala();
				vista.getPanelAnaydirSalasEnExposicionesPermanentesDadasDeAltaAdministrador().cargarSalas(vista.getpanelAnaydirSalasEnExposicionesPermanentesEnBorradorDisponiblesAdministrador().getExposicion());
	...
	public void agregarSala() {
		...
		if (selectedRow != -1) {
	        ...
	        for (Sala sala : sistema.getSalas()) {
	        	if(sala.esHoja()) {
	        		if (sala.getNombre().equals(nombreSala)) {
	        			Exposicion exposicion = vista.getPanelAnaydirSalasEnExposicionesPermanentesDadasDeAltaAdministrador().getExposicion();
