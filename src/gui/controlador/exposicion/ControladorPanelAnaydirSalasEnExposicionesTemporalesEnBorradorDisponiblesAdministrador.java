...
/**
 * Clase ControladorPanelAnaydirSalasEnExposicionesTemporalesEnBorradorDisponiblesAdministrador
 * 
 */
public class ControladorPanelAnaydirSalasEnExposicionesTemporalesEnBorradorDisponiblesAdministrador implements ActionListener {
	...
	/**
	 * Constructor ControladorPanelAnaydirSalasEnExposicionesTemporalesEnBorradorDisponiblesAdministrador
	 * @param vista (Ventana)
	 */
	public ControladorPanelAnaydirSalasEnExposicionesTemporalesEnBorradorDisponiblesAdministrador(Ventana vista) {
		this.vista = vista;
		table = vista.getPanelAnaydirSalasEnExposicionesTemporalesEnBorradorDisponiblesAdministrador().getTable();
		model = vista.getPanelAnaydirSalasEnExposicionesTemporalesEnBorradorDisponiblesAdministrador().getModel();

	}
	/**
	 * Metodo que define las acciones del PanelAnaydirSalasEnExposicionesTemporalesEnBorradorDisponiblesAdministrador
	 * @param e (ActionEvent)
	 */
	@Override
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand() == "Agregar") {
			if (table.getSelectedRow() == -1) {
				...
				agregarSala();
				vista.getPanelAnaydirSalasEnExposicionesTemporalesEnBorradorDisponiblesAdministrador().cargarSalas(vista.getPanelAnaydirSalasEnExposicionesTemporalesEnBorradorDisponiblesAdministrador().getExposicion());
	...
	public void agregarSala() {
		...
		if (selectedRow != -1) {
	        ...
	        for (Sala sala : sistema.getSalas()) {
	        	if(sala.esHoja()) {
	        		if (sala.getNombre().equals(nombreSala)) {
	        			Exposicion exposicion = vista.getPanelAnaydirSalasEnExposicionesTemporalesEnBorradorDisponiblesAdministrador().getExposicion();
